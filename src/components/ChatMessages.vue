<template>
  <main id="chatMessages" role="log" aria-live="polite" aria-atomic="false"
    class="absolute inset-0 overflow-y-auto p-5 bg-transparent z-10">
    <div class="space-y-4">
      <ChatWelcome v-if="!chat.messages.length" />

      <div v-else v-for="(msg, index) in chat.messages" :key="msg.timestamp || index"
        class="flex items-start gap-3 relative z-10" :class="msg.from === 'user' ? 'justify-end' : ''">
        <!-- رسائل البوت -->
        <template v-if="msg.from === 'bot' && !msg.loading">
          <img class="w-10 h-10 rounded-full shrink-0 bg-cover bg-center border border-[#d2961e]/30 shadow-sm"
            src="../assets/logo2.png" alt="صورة البوت" aria-hidden="true" />
          ...
          <div role="article"
            ...
            <template v-else-if="msg.headImage || msg.headCv">
              <div class="text-center">
                <img :src="msg.headImage" alt="رئيس الهيئة"
                ...
                <div class="mt-3">
                  <a href="/1528.pdf" download="CV-رئيس-الهيئة.pdf"
            ...
            <template v-else-if="msg.link">
              <span>{{ msg.text }} </span>
              <a :href="msg.link" target="_blank" rel="noopener" class="text-[#b07f14] underline">
                زور من هنا
              </a>
            </template>
            ...
            <template v-else-if="msg.branchCard">
              <div>
                ...
                <iframe :src="msg.branchMap" width="290" height="180" allowfullscreen="" loading="lazy"
                  class="mx-auto mb-3"></iframe>

                <a :href="msg.branchLink" target="_blank"
            ...
            <template v-else-if="msg.complaintForm">
              <div class="space-y-3 w-full max-w-[260px]">
                ...
                <input v-model="complaintNumber" type="text" placeholder="رقم الشكوى"
                  class="border border-[#d2961e]/40 rounded-xl px-3 py-2 w-full focus:outline-none focus:ring-1 focus:ring-[#d2961e]" />

                <input v-model="taxNumber" type="text" placeholder="الرقم الضريبي"
                  class="border border-[#d2961e]/40 rounded-xl px-3 py-2 w-full focus:outline-none focus:ring-1 focus:ring-[#d2961e]" />
  ...
  <div dir="auto" class="message-text">
    <ReadMore :text="msg.text" :maxChars="220" />
        ...
        <template v-else-if="msg.from === 'user'">
          ...
          <img class="w-10 h-10 rounded-full shrink-0 bg-cover bg-center border border-[#d2961e]/30 shadow-sm"
            src="../assets/user.webp" alt="صورة المستخدم" aria-hidden="true" />
        </template>
      </div>

      <!-- Loader -->
      <div v-if="chat.messages.some((m) => m.loading)" class="flex items-start gap-3 relative z-10">
        <img class="w-10 h-10 rounded-full shrink-0 bg-cover bg-center border border-[#d2961e]/30 shadow-sm"
          src="../assets/logo2.png" alt="chatbot typing" />
